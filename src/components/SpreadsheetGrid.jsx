...
export default function SpreadsheetGrid({ demandes = [], selectedId, onSelectQuery, agents = [], loading = false, onStatusChange, savingIds = new Set(), now = new Date() }) {
  ...
  return <div className="table-container"><table className="ops-table">
    ...
    <tbody>{!demandes.length ? <tr><td colSpan={8} className="table-empty-cell">Aucune demande dans cette vue.</td></tr> : demandes.map(d => {
      ...
      return <tr key={d.id} tabIndex={0} aria-selected={selectedId === d.id} className={`table-row ${selectedId === d.id ? 'is-selected' : ''}`} onClick={() => onSelectQuery(d.id)} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onSelectQuery(d.id) } }}>
        ...
        <td onClick={e => e.stopPropagation()}><StatusPill status={d.status} editable disabled={savingIds.has(d.id)} onChange={status => onStatusChange(d.id, { status })} /></td>
