    ...
    <div className="panel-header">
      <div className="panel-header-main">
        <div className="panel-identity">
          <div className="eyebrow">DEMANDE · {demande.initial_channel}</div>
    ...
    <div className="panel-body">
      <section className="panel-section">
        ...
        <div className="detail-fields">
          <div className="detail-field"><span>Statut</span><StatusPill status={demande.status} editable disabled={saving} onChange={status => onUpdate(demande.id, { status })} /></div>
          <label><span>Étape actuelle</span><select className="panel-select" disabled={saving} value={demande.current_stage || STAGES[0]} onChange={event => onUpdate(demande.id, { current_stage: event.target.value })}>{STAGES.map(stage => <option key={stage} value={stage}>{stage}</option>)}</select></label>
