...
- The Vite production build passed after these edits. Commit `57546a2` is pushed to `main`; GitHub shows the commit, but Cloudflare deployment completion is not verified. The pending migration still needs to be applied in Supabase before the new write path activates.
